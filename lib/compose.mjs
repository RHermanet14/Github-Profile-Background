...
/** GitHub Camo rejects images larger than ~5MB ("Content length exceeded"). */
export const GITHUB_CAMO_MAX_BYTES = 5 * 1024 * 1024;
/** Leave headroom under Camo's limit for the composed SVG response. */
export const SAFE_SVG_MAX_BYTES = 4 * 1024 * 1024;
/**
 * Max raw size for a single inlined raster (base64 expands ~4/3).
 * ~3MB leaves room for this project's other embeds (~66KB) under a 4MB SVG budget.
 */
export const MAX_INLINE_RASTER_BYTES = 3_100_000;
...
async function backgroundLayer(background, width, height) {
  ...
  assertInlineRasterSize(bytes, background);
...
function assertInlineRasterSize(bytes, label) {
  if (bytes.length <= MAX_INLINE_RASTER_BYTES) return;
  const kb = Math.round(bytes.length / 1024);
  const maxKb = Math.round(MAX_INLINE_RASTER_BYTES / 1024);
  throw new Error(
    `Image too large to embed for GitHub (${kb}KB > ${maxKb}KB): ${label}. Use a smaller PNG/JPEG (or a preset like "midnight"). Big GIFs exceed GitHub Camo's ~5MB limit after base64 encoding.`,
  );
}
...
async function renderSlot(slot) {
  ...
  if (type === "image") {
    ...
    const kind = sniffImageKind(bytes);
    if (kind === "html") {
      throw new Error(`Image slot URL returned HTML, not an image: ${slot.src}`);
    }
    assertInlineRasterSize(bytes, slot.src);
    const ext = isHttpUrl(slot.src)
      ? extname(new URL(slot.src).pathname) || guessExtFromUrl(slot.src)
      : extname(src);
    const mime = kind ? mimeForKind(kind, ext) : mimeForExt(ext);
    const href = `data:${mime};base64,${bytes.toString("base64")}`;
...
`;
  ...
  // #region agent log
  {
    const payload = {
      ...
      runId: "post-fix",
      hypothesisId: "A",
      location: "lib/compose.mjs:compose",
      message: "composed svg size",
      data: {
        bytes,
        overSafe: bytes > SAFE_SVG_MAX_BYTES,
        overCamo: bytes > GITHUB_CAMO_MAX_BYTES,
        maxInline: MAX_INLINE_RASTER_BYTES,
        background: String(config.background ?? "").slice(0, 80),
      },
      timestamp: Date.now(),
    };
    try {
      const { appendFileSync } = await import("node:fs");
      appendFileSync(join(ROOT, "debug-6300ce.log"), JSON.stringify(payload) + "\n");
    } catch {}
    fetch("http://127.0.0.1:7889/ingest/1b4116d1-63eb-4c81-b8a0-9ad82fed0b0e", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "X-Debug-Session-Id": "6300ce",
      },
  ...
  if (bytes > SAFE_SVG_MAX_BYTES) {
    throw new Error(
      `Composed SVG is ${Math.round(bytes / 1024)}KB; GitHub Camo rejects images over ~5MB (Content length exceeded). Use a small background (preset or <~1MB PNG/JPEG), not a large GIF.`,
    );
  }
